...
bool idAF::UpdateAnimation(void)
{
    ...
    for (i = 0; i < jointMods.Num(); i++)
    {
        ...
        origin =
            (bodyOrigin - jointMods[i].jointBodyOrigin * axis - renderOrigin) *
            renderAxis.Transpose();
        animator->SetAFPoseJointMod(jointMods[i].jointHandle,
                                    jointMods[i].jointMod, axis, origin);
    }
    animator->FinishAFPose(modifiedAnim,
...
idBounds idAF::GetBounds(void) const
{
    ...
    for (i = 0; i < jointMods.Num(); i++)
    {
        ...
        origin =
            (body->GetWorldOrigin() - entityOrigin) * entityAxis.Transpose();
        axis = body->GetWorldAxis() * entityAxis.Transpose();
        b.FromTransformedBounds(body->GetClipModel()->GetBounds(), origin,
                                axis);
    ...
    for (i = 0; i < jointMods.Num(); i++)
    {
        ...
        body->SetWorldOrigin(renderEntity->origin +
                             (origin + jointMods[i].jointBodyOrigin * axis) *
                                 renderEntity->axis);
        body->SetWorldAxis(jointMods[i].jointBodyAxis * axis *
                           renderEntity->axis);
    }
...
void idAF::ChangePose(idEntity* ent, int time)
{
    ...
    for (i = 0; i < jointMods.Num(); i++)
    {
        ...
        body->SetWorldAxis(jointMods[i].jointBodyAxis * axis *
                           renderEntity->axis);
        body->SetLinearVelocity((body->GetWorldOrigin() - lastOrigin) *
                                invDelta);
    }
    ...
    for (i = 0; i < jointMods.Num(); i++)
    {
        ...
        for (j = 0; j < numClipModels; j++)
        {
            ...
            if (gameLocal.clip.ContentsModel(
                    ...
                    body->GetWorldAxis(), -1, cm->Handle(), cm->GetOrigin(),
                    cm->GetAxis()))
            {
...
bool idAF::LoadBody(const idDeclAF_Body* fb, const idJointMat* joints)
{
    ...
    // update table to find the nearest articulated figure body for a joint of
    // the skeletal model
...
bool idAF::LoadConstraint(const idDeclAF_Constraint* fc)
{
    ...
    switch (fc->type)
    {
        ...
        case DECLAF_CONSTRAINT_BALLANDSOCKETJOINT:
        {
            ...
            {
                c = new idAFConstraint_BallAndSocketJoint(fc->name, body1,
                                                          body2);
                physicsObj.AddConstraint(c);
            }
        ...
        case DECLAF_CONSTRAINT_HINGE:
        {
            ...
            switch (fc->limit)
            {
                case idDeclAF_Constraint::LIMIT_CONE:
                {
                    ...
                    axis = left * idRotation(vec3_origin, fc->axis.ToVec3(),
                                             fc->limitAngles[0]);
        ...
        case DECLAF_CONSTRAINT_SPRING:
        {
            ...
            c->SetSpring(fc->stretch, fc->compress, fc->damping,
                         fc->restLength);
...
bool idAF::Load(idEntity* ent, const char* fileName)
{
    ...
    if (file->bodies.Num() == 0 || file->bodies[0]->jointName != "origin")
    {
        gameLocal.Warning("idAF::Load: articulated figure '%s' for entity '%s' "
                          "at (%s) has no body which modifies the origin "
                          "joint.",
                          name.c_str(), ent->name.c_str(),
                          ent->GetPhysics()->GetOrigin().ToString(0));
        return false;
    ...
    {
        gameLocal.Warning("idAF::Load: articulated figure '%s' for entity '%s' "
                          "at (%s) has no or defaulted modelDef '%s'",
                          name.c_str(), ent->name.c_str(),
                          ent->GetPhysics()->GetOrigin().ToString(0),
    ...
    {
        gameLocal.Warning("idAF::Load: articulated figure '%s' for entity '%s' "
                          "at (%s) has no or defaulted model '%s'",
                          name.c_str(), ent->name.c_str(),
                          ent->GetPhysics()->GetOrigin().ToString(0),
                          model ? model->Name() : "");
        return false;
    ...
    {
        gameLocal.Warning("idAF::Load: articulated figure '%s' for entity '%s' "
                          "at (%s) has no modified animation '%s'",
                          name.c_str(), ent->name.c_str(),
                          ent->GetPhysics()->GetOrigin().ToString(0),
    ...
    gameEdit->ANIM_CreateAnimFrame(
        model, animator->GetAnim(modifiedAnim)->MD5Anim(0), numJoints, joints,
        1, animator->ModelDef()->GetVisualOffset(), animator->RemoveOrigin());
    ...
    physicsObj.SetSuspendSpeed(file->suspendVelocity,
                               file->suspendAcceleration);
    ...
    // delete any constraints in the physicsObj that are no longer in the
    // idDeclAF
    ...
    for (i = 0; i < animator->NumJoints(); i++)
    {
        if (jointBody[i] == -1)
        {
            gameLocal.Warning("idAF::Load: articulated figure '%s' for entity "
                              "'%s' at (%s) joint '%s' is not contained by a "
                              "body",
...
bool idAF::TestSolid(void) const
{
    ...
    for (i = 0; i < physicsObj.GetNumBodies(); i++)
    {
        ...
        if (gameLocal.clip.Translation(
                trace, body->GetWorldOrigin(), body->GetWorldOrigin(),
                body->GetClipModel(), body->GetWorldAxis(), body->GetClipMask(),
                self))
        {
            float depth =
                idMath::Fabs(trace.c.point * trace.c.normal - trace.c.dist);

            body->SetWorldOrigin(body->GetWorldOrigin() +
                                 trace.c.normal * (depth + 8.0f));

            gameLocal.DWarning("%s: body '%s' stuck in %d (normal = %.2f %.2f "
                               "%.2f, depth = %.2f)",
                               self->name.c_str(), body->GetName().c_str(),
                               trace.c.contents, trace.c.normal.x,
                               trace.c.normal.y, trace.c.normal.z, depth);
            solid = true;
        }
    }
    return solid;
...
void idAF::StartFromCurrentPose(int inheritVelocityTime)
{
    ...
    {
        // transform the articulated figure to reflect the current animation
        // pose
        SetupPose(self, gameLocal.time);
    }
...
void idAF::SetConstraintPosition(const char* name, const idVec3& pos)
{
    ...
    {
        gameLocal.Warning("constraint '%s' does not bind to another entity",
                          name);
        return;
    }
    ...
    switch (constraint->GetType())
    {
        ...
        default:
        {
            gameLocal.Warning("cannot set the constraint position for '%s'",
                              name);
            break;
        }
    }
...
void idAF::AddBindConstraints(void)
{
         ...
         kv = args.MatchPrefix("bindConstraint ", kv))
    {
        ...
        else if (type.Icmp("ballAndSocket") == 0)
        {
            ...
            {
                gameLocal.Warning(
                    "idAF::AddBindConstraints: joint '%s' not found",
                    jointName.c_str());
            }
        ...
        {
            gameLocal.Warning("idAF::AddBindConstraints: unknown constraint "
                              "type '%s' on entity '%s'",
